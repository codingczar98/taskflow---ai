    ...
    try {
        return await databases.listDocuments(APPWRITE_DATABASE_ID, 'tasks',
            [
                ...
                Query.and([
                    Query.greaterThanEqual('due_date', startOfToday().toISOString()), //get tasks due today
                    Query.lessThan('due_date', startOfTomorrow().toISOString()) //get tasks due tomorrow
                ]) //get tasks due today
            ]
        )
